...
export function AdminProfilesPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      const { data, error: requestError } = await listProfiles(query);
      if (ignore) return;
      if (requestError) {
  ...
  }, [configured, query, session]);

  const filteredCountLabel = useMemo(() => {
    if (!query.trim()) return `${profiles.length} total`;
    return `${profiles.length} match${profiles.length === 1 ? "" : "es"}`;
  }, [profiles.length, query]);
              ...
              {loading || sessionLoading ? (
                ...
                profiles.map((profile) => (
                  <tr key={profile.id} className="border-t border-white/8 text-sm text-white/80">
                    ...
                    <td className="px-5 py-4">{profile.company ?? "—"}</td>
